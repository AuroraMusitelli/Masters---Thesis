  ...
  decisioni <- tabella %>%
    dplyr::mutate(
      Regola = dplyr::case_when(
        Stationary_Drift==FALSE & Stationary_Trend==FALSE ~ "diff",     # se la serie non è stazionaria né con drift né con trend -> differenziazione 
        Stationary_Drift==FALSE & Stationary_Trend==TRUE  ~ "detrend",  # trend deterministico -> detrendizzazione
        Stationary_Drift==TRUE  & Stationary_Trend==TRUE  ~ "none",     # se è già stazionaria -> nessuna trasformazione
        TRUE ~ "none"))
  df_out <- df
  trasformazioni <- list()
